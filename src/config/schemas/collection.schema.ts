...
export const collectionPanelRefSchema = z.object({
  panelId: z.string().min(1, "Campo obrigatório"),
  /** Sobrescreve o `timerSeconds` da coleção só para este painel. */
  timerSeconds: z.number().int().positive("Informe um número maior que zero").optional(),
});
...
export const collectionConfigSchema = z.object({
  ...
  timerSeconds: z.number().int().positive("Informe um número maior que zero").default(45),
  /** A cada N voltas completas na coleção, os painéis são remontados (dados/iframes atualizados). */
  refreshEveryCycles: z.number().int().positive("Informe um número maior que zero").default(1),
  panels: z.array(collectionPanelRefSchema).min(1, "Adicione pelo menos um painel"),
});
